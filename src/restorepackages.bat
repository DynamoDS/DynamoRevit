...
REM 2. download 3rdParty packages by Aget.exe
    ...
    :: Symlinks are generated here (first try with NET80)
    %PythonAget% -agettable "%ConfigDir%\packages.aget" -refsDir "%SymLinksDir%"
    if ERRORLEVEL 1 (
        echo ERROR: Failed with NET80, retrying with net8.0...
        ...
        %PythonAget% -agettable "%ConfigDir%\packages.aget" -refsDir "%SymLinksDir%"
        if ERRORLEVEL 1 (
            echo ERROR: Failed to update Dynamo 3rdParty nuget packages in packages.aget with net8.0 as well
